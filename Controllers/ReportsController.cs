...
namespace LawFirmManagement.Controllers
{
    ...
    public class ReportsController : Controller
    {
        ...
        public async Task<IActionResult> Download(string? reportType, DateTime? startDate, DateTime? endDate)
        {
            ...
            sb.Append("th { background-color: #2c3e50; color: white; padding: 10px; }");
            ...
            {
                sb.Append("<h3>Case History</h3><table><tr><th>Title</th><th>Type</th><th>Start Date</th><th>Status</th></tr>");
            ...
            {
                sb.Append("<h3>Financial Transactions</h3><table><tr><th>Date</th><th>Case</th><th>Method</th><th>Amount</th></tr>");
                foreach (var p in model.PaymentList)
                {
                    decimal amount = User.IsInRole("Lawyer") ? p.LawyerShare : p.TotalAmount;
                    sb.Append($"<tr><td>{p.PaymentDate:dd-MMM-yyyy}</td><td>{p.Case?.CaseTitle ?? "N/A"}</td><td>{p.PaymentMethod}</td><td>{amount:N2}</td></tr>");
                }
                sb.Append("</table>");
            }
            ...
            return File(Encoding.UTF8.GetBytes(sb.ToString()), "text/html",
                $"Report_{model.ReportType}_{DateTime.Now:yyyyMMdd}.html");
        }
        ...
        private async Task<ReportViewModel> GetReportData(string? reportType, DateTime? startDate, DateTime? endDate)
        {
            ...
            var hearingsQuery = _db.Hearings
                .Include(h => h.Case)
                .Where(h => h.HearingDate >= start && h.HearingDate <= end);

            if (userRole == "Lawyer")
            {
                casesQuery = casesQuery.Where(c => c.LawyerId == userId);
                paymentsQuery = paymentsQuery.Where(p => p.Case != null && p.Case.LawyerId == userId);
                hearingsQuery = hearingsQuery.Where(h => h.Case != null && h.Case.LawyerId == userId);
            }
            else if (userRole == "Client")
            {
                casesQuery = casesQuery.Where(c => c.ClientId == userId);
                paymentsQuery = paymentsQuery.Where(p => p.Case != null && p.Case.ClientId == userId);
                hearingsQuery = hearingsQuery.Where(h => h.Case != null && h.Case.ClientId == userId);
            }

            var caseList = await casesQuery.ToListAsync();
            var paymentList = await paymentsQuery.ToListAsync();
            var hearingList = await hearingsQuery.ToListAsync();

            decimal totalRevenue = userRole == "Lawyer"
                ? paymentList.Sum(p => p.LawyerShare)
                : paymentList.Sum(p => p.TotalAmount);
